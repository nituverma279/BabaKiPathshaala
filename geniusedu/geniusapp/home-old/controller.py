    ...
    if session.get('subs_pac_id'):
        if request.method=='POST':
            ...
            try:
                ...
                return render_template('home/payment-offline.html',payamount=amount)
            except Exception as e:
                app.logger.error(str(e))
                return abort(500)
    else:
...
def seminar_topics_list(cano_url):
    try:
        ...
        if seminar:
            ...
            return render_template('seminar/seminar-topic.html', seminar=seminar,
                                   seminar_topic_details=seminar_topic_details)
        else:
            return redirect('/404')
    except Exception as e:
        app.logger.error(str(e))
    ...
    try:
        ...
        if seminar:
            ...
            if not joined_sem:
                ...
                flash('You have alreay joined the seminar ({})'.format(seminar.title), 'danger')
                return redirect(url_for('dashboard'))
        else:
