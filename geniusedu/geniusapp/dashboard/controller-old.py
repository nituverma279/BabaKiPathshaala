...
def dashboard():
    ...
    # if student
    if current_user.user_role_id==4: 
        stud_pac_sub =  db.session.execute("SELECT student_package_subscription.*,pac_course.seo_title,pac_course.cano_url,pac_course.cover_banner,\
        ...
        if stu_subs_package:
            ...
            for stu_subs in stu_subs_package: 
                subs_months_list = Student_subs_pac_months.query.filter_by(stu_pac_subs_id=stu_subs.id,subs_month=current_month).all()
                if not subs_months_list:
...
@app.route('/dashboard/live-class/<cano_url>')
@login_required
@student_required
def subscription_live_class(cano_url):

    try:
        package_details = Pac_course.query.filter_by(cano_url=cano_url,is_active=True).first()

        if package_details:
            student_sub_pac = Student_package_subscription.query.filter_by(package_id=package_details.id, student_id=current_user.id,\
    ...
    try:
        ...
        if sem_list:
            for sem in sem_list:
                ...
                if seminar:
                    seminars_list.append({'seminar_id': seminar.id, 'title': seminar.title, 'canonical_url': seminar.canonical_url, \
