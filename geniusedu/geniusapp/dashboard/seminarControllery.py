...
Student_package_subscription,Student_subs_pac_months,Student_subs_pac_optional,Months,Subscription_trans_log,Seminars, \
    Seminar_details,Seminar_attend,Sem_broad_streams_records,Sem_chat,Ban_sem_chat,Seminar_start_teachers
    ...
    try:
        ...
        if seminar:
            ...
            if seminar_topic:
                seminar = Seminars.query.filter_by(canonical_url=cano_url, is_active=True).first()
                """ for chat app  """
                sem_start_teach_exists = Seminar_start_teachers.query.filter_by(seminar_id=seminar.id).first()
                if sem_start_teach_exists:
                    sem_start_teach_exists.teacher_id = current_user.id
                    db.session.commit()
                else:
                    sem_tech=Seminar_start_teachers(seminar_id=seminar.id, teacher_id=current_user.id)
                    db.session.add(sem_tech)
                    db.session.commit()
...
def watch_live_seminar(cano_url):
    try:
        ...
        if seminar:
            sem_attend = Seminar_attend.query.filter_by(seminar_id=seminar.id, student_id=current_user.id,is_approved=True).first()
            if sem_attend:
                chat_history = Sem_chat.query.filter_by(seminar_id=seminar.id).all()
                ...
                flash('Sorry, You are not allowed to watch seminar. Please contact with support team.','danger')
                resp = make_response(redirect(url_for('dashboard')))
                return resp
        else:
...
""" student can watch live seminar  """
@app.route('/dashboard/mb-watch-live-seminar/<string:cano_url>')
@login_required
@student_required
def mb_watch_live_seminar(cano_url):
    try:
        seminar = Seminars.query.filter_by(canonical_url=cano_url,is_active=1).first()
        if seminar:
            sem_attend = Seminar_attend.query.filter_by(seminar_id=seminar.id, student_id=current_user.id,is_approved=True).first()
            if sem_attend:
                ...
                if sem_start_teacher:
                    banned_user_form_chat = Ban_sem_chat.query.filter_by(seminar_id=seminar.id,banned_user_id=current_user.id).first()
                    broadcast_video_stream_record= Sem_broad_streams_records.query.filter_by(seminar_id=seminar.id).order_by(Sem_broad_streams_records.id.desc()).first()
                    resp = make_response(render_template('dashboard/seminar/mb-watch-live-seminar.html',seminar=seminar,banned_user_form_chat=banned_user_form_chat,\
                        broadcast_video_stream_record=broadcast_video_stream_record,chat_history=chat_history,sem_start_teacher=sem_start_teacher))
                    return resp
                else:
                    flash('Sorry, Please wait until seminar is not started.','danger')
